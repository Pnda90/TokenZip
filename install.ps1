# tokenzip — smart multi-agent installer (Windows / PowerShell).
#
# One line:
...
$InitScriptUrl = "$RawBase/tools/tokenzip-init.js"
$McpShrinkPkg = "tokenzip-shrink"

# ── Help ────────────────────────────────────────────────────────────────────
if ($Help) {
@"
tokenzip installer (Windows) — detects your agents and installs tokenzip for each.
...
FLAGS
  ...
  -WithMcpShrink   Claude Code: register tokenzip-shrink MCP proxy. ON by default.
...
URLS THE INSTALLER MAY FETCH FROM
  ...
  $RawBase/tools/tokenzip-init.js
"@ | Write-Host
  exit 0
}
...
function Install-Claude {
  ...
  if (-not $Force) {
    try {
      ...
      if ($list -match "(?i)tokenzip") { $alreadyInstalled = $true }
    } catch {}
  }
  if ($alreadyInstalled) {
    ...
    if ((Try-Run "claude" @("plugin", "marketplace", "add", $Repo)) -and
        (Try-Run "claude" @("plugin", "install", "tokenzip@tokenzip"))) {
    ...
    if ($DryRun) {
      ...
      try {
        if ($localPs1) {
          ...
          $tmp = Join-Path $env:TEMP "tokenzip-hooks-install-$([Guid]::NewGuid()).ps1"
  ...
  # registering an `npx -y tokenzip-shrink` entry that 404s on every spawn.
  if ($WithMcpShrink) {
    Say "  → wiring tokenzip-shrink MCP proxy (-WithMcpShrink)"
    if (Has-Cmd "npm") {
      $packageOnNpm = $false
      ...
      if (-not $packageOnNpm) {
        ...
        Record-Skipped "tokenzip-shrink" "npm registry probe failed"
        Write-Host ""
        return
      }
    }
    ...
    if ($hasMcpAdd) {
      if ($DryRun) {
        Note "    would run: claude mcp add tokenzip-shrink -- npx -y $McpShrinkPkg"
      } else {
        if (Try-Run "claude" @("mcp", "add", "tokenzip-shrink", "--", "npx", "-y", $McpShrinkPkg)) {
          Record-Installed "tokenzip-shrink"
          ...
          Record-Failed "tokenzip-shrink" "claude mcp add failed"
        }
      }
    } else {
      ...
      Write-Host '    }'
      Write-Host ""
      Record-Skipped "tokenzip-shrink" "manual config required (snippet printed)"
    }
  }
  Write-Host ""
}
...
function Run-Init {
  ...
  if ($RepoRoot -and (Test-Path (Join-Path $RepoRoot "tools\tokenzip-init.js")) -and (Has-Cmd "node")) {
    if (Try-Run "node" (@((Join-Path $RepoRoot "tools\tokenzip-init.js")) + $initArgs)) { return $true }
    return $false
  }
  ...
  $tmp = Join-Path $env:TEMP "tokenzip-init-$([Guid]::NewGuid()).js"
  try {
...
if ($WithInit) {
  ...
  if (Run-Init) {
    Record-Installed "tokenzip-init ($PWD)"
  } else {
    Record-Failed "tokenzip-init" "tools/tokenzip-init.js failed"
  }
  Write-Host ""
