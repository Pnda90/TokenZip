# tokenzip — one-command hook installer for Claude Code (Windows PowerShell)
...
#        irm https://raw.githubusercontent.com/JuliusBrussee/tokenzip/main/hooks/install.ps1 | iex
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Host "ERROR: 'node' is required to install the tokenzip hooks (used to merge" -ForegroundColor Red
...
$RepoUrl = "https://raw.githubusercontent.com/JuliusBrussee/tokenzip/main/hooks"

$HookFiles = @("package.json", "tokenzip-config.js", "tokenzip-activate.js", "tokenzip-mode-tracker.js", "tokenzip-stats.js", "tokenzip-statusline.sh", "tokenzip-statusline.ps1")
...
if (-not $Force) {
    ...
    if ($AllFilesPresent -and (Test-Path $Settings)) {
        try {
                ...
                foreach ($entry in $entries) {
                    if ($entry.hooks) {
                        foreach ($hookDef in $entry.hooks) {
                            if ($hookDef.command -and $hookDef.command.Contains("tokenzip")) {
                                return $true
                            }
                        }
                    }
            ...
            $HooksWired = (& $hasTokenZipHook "SessionStart") -and (& $hasTokenZipHook "UserPromptSubmit")
    ...
    if ($AllFilesPresent -and $HooksWired -and $HasStatusLine) {
        Write-Host "TokenZip hooks already installed in $HooksDir"
...
if ($Force -and (Test-Path (Join-Path $HooksDir "tokenzip-activate.js"))) {
    Write-Host "Reinstalling tokenzip hooks (-Force)..."
} else {
    Write-Host "Installing tokenzip hooks..."
}
...
$env:TOKENZIP_SETTINGS = $Settings -replace '\\', '/'
$env:TOKENZIP_HOOKS_DIR = $HooksDir -replace '\\', '/'

$nodeScript = @'
const fs = require('fs');
const settingsPath = process.env.TOKENZIP_SETTINGS;
const hooksDir = process.env.TOKENZIP_HOOKS_DIR;
const managedStatusLinePath = hooksDir + '/tokenzip-statusline.ps1';
...
$tmpScript = Join-Path $env:TEMP "tokenzip-install-$([System.Diagnostics.Process]::GetCurrentProcess().Id).js"
try {
...
Write-Host "  - SessionStart hook: auto-loads tokenzip rules every session"
Write-Host "  - Mode tracker hook: updates statusline badge when you switch modes"
Write-Host "    (/tokenzip lite, /tokenzip ultra, /tokenzip-commit, etc.)"
Write-Host "  - Statusline badge: shows [TOKENZIP] or [TOKENZIP:ULTRA] etc."
