...
$Flag = Join-Path $ClaudeDir ".tokenzip-active"
...
if ([string]::IsNullOrEmpty($Mode) -or $Mode -eq "full") {
    [Console]::Write("${Esc}[38;5;172m[TOKENZIP]${Esc}[0m")
} else {
    $Suffix = $Mode.ToUpperInvariant()
    [Console]::Write("${Esc}[38;5;172m[TOKENZIP:$Suffix]${Esc}[0m")
}

# Savings suffix: on by default. Opt out via TOKENZIP_STATUSLINE_SAVINGS=0.
# Reads a pre-rendered string written by tokenzip-stats.js. Refuses reparse
# points and strips control bytes (matches statusline.sh hardening). Until
# /tokenzip-stats has run at least once, the suffix file is absent and nothing
# is rendered — safe default for fresh installs.
if ($env:TOKENZIP_STATUSLINE_SAVINGS -ne "0") {
    $SavingsFile = Join-Path $ClaudeDir ".tokenzip-statusline-suffix"
