#!/bin/bash
# tokenzip — one-command hook installer for Claude Code
# Installs: SessionStart hook (auto-load rules) + UserPromptSubmit hook (mode tracking)
# Usage: bash hooks/install.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/JuliusBrussee/tokenzip/main/hooks/install.sh)
...
if ! command -v node >/dev/null 2>&1; then
  echo "ERROR: 'node' is required to install the tokenzip hooks (used to merge"
...
REPO_URL="https://raw.githubusercontent.com/JuliusBrussee/tokenzip/main/hooks"

HOOK_FILES=("package.json" "tokenzip-config.js" "tokenzip-activate.js" "tokenzip-mode-tracker.js" "tokenzip-stats.js" "tokenzip-statusline.sh")
...
if [ "$FORCE" -eq 0 ]; then
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ -f "$SETTINGS" ]; then
    if TOKENZIP_SETTINGS="$SETTINGS" node -e "
      const fs = require('fs');
      const settings = JSON.parse(fs.readFileSync(process.env.TOKENZIP_SETTINGS, 'utf8'));
      const hasTokenZipHook = (event) =>
        Array.isArray(settings.hooks?.[event]) &&
        settings.hooks[event].some(e =>
          e.hooks && e.hooks.some(h => h.command && h.command.includes('tokenzip'))
        );
      process.exit(
        hasTokenZipHook('SessionStart') &&
        hasTokenZipHook('UserPromptSubmit') &&
        !!settings.statusLine
          ? 0
          : 1
      );
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ "$HOOKS_WIRED" -eq 1 ] && [ "$HAS_STATUSLINE" -eq 1 ]; then
    ALREADY_INSTALLED=1
    echo "TokenZip hooks already installed in $HOOKS_DIR"
...
if [ "$FORCE" -eq 1 ] && [ -f "$HOOKS_DIR/tokenzip-activate.js" ]; then
  echo "Reinstalling tokenzip hooks (--force)..."
else
  echo "Installing tokenzip hooks..."
fi
...
chmod +x "$HOOKS_DIR/tokenzip-statusline.sh"
...
TOKENZIP_SETTINGS="$SETTINGS" TOKENZIP_HOOKS_DIR="$HOOKS_DIR" node -e "
  const fs = require('fs');
  const settingsPath = process.env.TOKENZIP_SETTINGS;
  const hooksDir = process.env.TOKENZIP_HOOKS_DIR;
  const managedStatusLinePath = hooksDir + '/tokenzip-statusline.sh';
  const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
  if (!settings.hooks) settings.hooks = {};

  // SessionStart — auto-load tokenzip rules
  ...
  if (!hasStart) {
    settings.hooks.SessionStart.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/tokenzip-activate.js\"',
        timeout: 5,
        statusMessage: 'Loading tokenzip mode...'
      }]
    });
  }

  // UserPromptSubmit — track mode changes when user types /tokenzip commands
  ...
  if (!hasPrompt) {
    settings.hooks.UserPromptSubmit.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/tokenzip-mode-tracker.js\"',
        timeout: 5,
        statusMessage: 'Tracking tokenzip mode...'
      }]
    });
  }

  // Statusline — wire tokenzip badge (report if skipped)
  if (!settings.statusLine) {
    ...
    if (cmd.includes(managedStatusLinePath)) {
      ...
      console.log('  NOTE: Existing statusline detected — tokenzip badge NOT added.');
...
echo "  - SessionStart hook: auto-loads tokenzip rules every session"
echo "  - Mode tracker hook: updates statusline badge when you switch modes"
echo "    (/tokenzip lite, /tokenzip ultra, /tokenzip-commit, etc.)"
echo "  - Statusline badge: shows [TOKENZIP] or [TOKENZIP:ULTRA] etc."
