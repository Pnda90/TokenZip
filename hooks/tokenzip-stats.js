#!/usr/bin/env node
// tokenzip-stats — read the active Claude Code session log, print real token
// usage plus an estimated savings figure from the benchmark in benchmarks/.
//
// Run directly:    node hooks/tokenzip-stats.js
// Inside Claude:   /tokenzip-stats triggers this via the UserPromptSubmit hook.
...
const os = require('os');
const { readFlag, appendFlag, readHistory, safeWriteFlag } = require('./tokenzip-config');
...
// Detect *.original.md / *.md pairs left behind by tokenzip-compress. The
...
function formatHistory({ sessions, outputTokens, estSavedTokens, estSavedUsd, since }) {
  ...
  if (sessions === 0) {
    return `\nTokenZip Stats — Lifetime${window}\n${sep}\nNo sessions logged yet — run /tokenzip-stats inside any session to start tracking.\n${sep}\n`;
  }
...
function formatShare({ outputTokens, turns, mode, model }) {
  if (turns === 0) {
    return '⚡ tokenzip armed but no turns yet — tokenzip.sh';
  }
  const ratio = COMPRESSION[mode] != null ? COMPRESSION[mode] : null;
  const price = priceForModel(model);
  ...
  if (ratio !== null) {
    ...
    return `⚡ Saved ${estSaved.toLocaleString()} output tokens${usd} across ${turns} turns this session — tokenzip.sh`;
  }
  return `⚡ ${turns} turns, ${outputTokens.toLocaleString()} output tokens this session — tokenzip.sh`;
}
...
function formatStats({ outputTokens, cacheReadTokens, turns, mode, model, sessionPath, compressed }) {
  ...
  if (ratio !== null) {
    ...
    savings = `Est. without tokenzip:  ${estNormal.toLocaleString()}\n` +
    ...
    savings = 'TokenZip not active this session.';
  }
...
function main() {
  ...
  const historyPath = path.join(claudeDir, '.tokenzip-history.jsonl');
  ...
  if (all || sinceArg) {
    ...
    if (sinceArg && sinceMs === null) {
      process.stderr.write(`tokenzip-stats: --since takes Nh or Nd (e.g. 7d, 24h), got: ${sinceArg}\n`);
      process.exit(2);
    }
  ...
  if (!sessionFile) {
    process.stderr.write('tokenzip-stats: no Claude Code session found.\n');
    process.exit(1);
  }

  const parsed = parseSession(sessionFile);
  const mode = readFlag(path.join(claudeDir, '.tokenzip-active'));

  // Append a snapshot of this session's totals to the lifetime log. Multiple
  // /tokenzip-stats calls in one session emit multiple lines for the same
  ...
  if (parsed.turns > 0) {
    ...
    // cat without parsing JSONL. Updated on every /tokenzip-stats run.
    // Routed through safeWriteFlag — the suffix path is predictable and
    // user-owned, same symlink-clobber surface as the .tokenzip-active flag.
    const agg = aggregateHistory(historyPath, null);
    const suffix = agg.estSavedTokens > 0 ? `⛏ ${humanizeTokens(agg.estSavedTokens)}` : '';
    safeWriteFlag(path.join(claudeDir, '.tokenzip-statusline-suffix'), suffix);
  }
