#!/usr/bin/env node
// tokenzip init — drop the always-on tokenzip activation rule into a target
// repo for every IDE agent we support. Idempotent. Safe to re-run.
//
// Usage:
//   node tools/tokenzip-init.js [target-dir] [--dry-run] [--force] [--only <agent>]
//   curl -fsSL https://raw.githubusercontent.com/JuliusBrussee/tokenzip/main/tools/tokenzip-init.js | node - [args]
//
...
// existing memory files — that's the job of `/tokenzip:compress`.

const fs = require('fs');
const path = require('path');

// Embedded so the tool works standalone (npx-style) without the rules/ dir.
// Mirrors rules/tokenzip-activate.md verbatim — keep these in sync.
const RULE_BODY = `Respond terse like smart tokenzip. All technical substance stay. Only fluff die.
...
Switch level: /tokenzip lite|full|ultra|wenyan
Stop: "stop tokenzip" or "normal mode"

Auto-Clarity: drop tokenzip for security warnings, irreversible actions, user confused. Resume after.

Boundaries: code/commits/PRs written normal.
`;

const SENTINEL = 'Respond terse like smart tokenzip';

const AGENTS = [
  { id: 'cursor',   file: '.cursor/rules/tokenzip.mdc',
    frontmatter: '---\ndescription: "TokenZip mode — terse communication, ~75% fewer tokens, full technical accuracy"\nalwaysApply: true\n---\n\n',
    mode: 'replace' },
  { id: 'windsurf', file: '.windsurf/rules/tokenzip.md',
    frontmatter: '---\ntrigger: always_on\n---\n\n',
    mode: 'replace' },
  { id: 'cline',    file: '.clinerules/tokenzip.md',
    frontmatter: '',
    mode: 'replace' },
...
function loadRuleBody() {
  ...
  try {
    const local = path.join(__dirname, '..', 'rules', 'tokenzip-activate.md');
...
function help() {
  console.log(`tokenzip init — drop always-on tokenzip rule into a target repo

Usage: tokenzip-init.js [target-dir] [--dry-run] [--force] [--only <agent>]
...
function main() {
  ...
  console.log(`⚡ tokenzip init — ${opts.target}${opts.dryRun ? ' (dry run)' : ''}\n`);
