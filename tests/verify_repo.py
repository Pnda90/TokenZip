#!/usr/bin/env python3
"""Local verification runner for tokenzip install surfaces."""
...
def verify_manifests_and_syntax() -> None:
    ...
    manifest_paths = [
        ...
        ROOT / "plugins/tokenzip/.codex-plugin/plugin.json",
    ]
    ...
    ensure("tokenzip-config.js" in install_sh, "install.sh missing tokenzip-config.js")
    ensure("tokenzip-config.js" in uninstall_sh, "uninstall.sh missing tokenzip-config.js")
...
def verify_powershell_static() -> None:
    ...
    statusline_text = (ROOT / "hooks/tokenzip-statusline.ps1").read_text(encoding="utf-8")

    ensure("tokenzip-config.js" in install_text, "install.ps1 missing tokenzip-config.js")
    ensure("tokenzip-config.js" in uninstall_text, "uninstall.ps1 missing tokenzip-config.js")
    ensure("tokenzip-statusline.ps1" in install_text, "install.ps1 missing statusline.ps1")
    ensure("tokenzip-statusline.ps1" in uninstall_text, "uninstall.ps1 missing statusline.ps1")
    ...
    ensure("[TOKENZIP" in statusline_text, "tokenzip-statusline.ps1 missing badge output")

    print("Windows install path statically wired")


def load_compress_modules():
    sys.path.insert(0, str(ROOT / "tokenzip-compress"))
...
def verify_compress_fixtures() -> None:
    ...
    fixtures = sorted((ROOT / "tests/tokenzip-compress").glob("*.original.md"))
    ensure(fixtures, "No tokenzip-compress fixtures found")
    ...
    print(f"Validated {len(fixtures)} tokenzip-compress fixture pairs")
...
def verify_hook_install_flow() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="tokenzip-verify-") as temp_root:
        ...
        activate = run(
            ["node", "hooks/tokenzip-activate.js"],
            env=hook_env,
        )
        ensure("TOKENZIP MODE ACTIVE" in activate.stdout, "activation output missing tokenzip banner")
        ensure("STATUSLINE SETUP NEEDED" not in activate.stdout, "activation should stay quiet when custom statusline exists")
        ensure((claude_dir / ".tokenzip-active").read_text(encoding="utf-8") == "full", "activation flag should default to full")

        # Test configurable default mode via TOKENZIP_DEFAULT_MODE env var
        activate_custom = run(
            ["node", "hooks/tokenzip-activate.js"],
            env={**hook_env, "TOKENZIP_DEFAULT_MODE": "ultra"},
        )
        ensure("TOKENZIP MODE ACTIVE" in activate_custom.stdout, "activation with custom default missing banner")
        ensure(
            (claude_dir / ".tokenzip-active").read_text(encoding="utf-8") == "ultra",
            "TOKENZIP_DEFAULT_MODE=ultra should set flag to ultra",
        )
        # Test "off" mode — activation skipped, flag removed
        activate_off = run(
            ["node", "hooks/tokenzip-activate.js"],
            env={**hook_env, "TOKENZIP_DEFAULT_MODE": "off"},
        )
        ensure("TOKENZIP MODE ACTIVE" not in activate_off.stdout, "off mode should not emit tokenzip banner")
        ensure(not (claude_dir / ".tokenzip-active").exists(), "off mode should remove flag file")

        # Test mode tracker with /tokenzip when default is off — should NOT write flag
        subprocess.run(
            ["node", "hooks/tokenzip-mode-tracker.js"],
            cwd=ROOT,
            env={**os.environ, **hook_env, "TOKENZIP_DEFAULT_MODE": "off"},
            text=True,
            encoding="utf-8",
            input='{"prompt":"/tokenzip"}',
            capture_output=True,
            check=True,
        )
        ensure(not (claude_dir / ".tokenzip-active").exists(), "/tokenzip with off default should not write flag")

        # Reset back to full for subsequent tests
        (claude_dir / ".tokenzip-active").write_text("full")
        ...
        ultra_prompt = subprocess.run(
            ...
            text=True,
            encoding="utf-8",
            input='{"prompt":"/tokenzip ultra"}',
            capture_output=True,
            check=True,
        )
        ensure(
            "TOKENZIP MODE ACTIVE (ultra)" in ultra_prompt.stdout,
            "mode tracker should emit active-mode reinforcement",
        )
        ensure((claude_dir / ".tokenzip-active").read_text(encoding="utf-8") == "ultra", "mode tracker did not record ultra")
        ...
        ensure(not (claude_dir / ".tokenzip-active").exists(), "normal mode should remove flag file")

        (claude_dir / ".tokenzip-active").write_text("wenyan-ultra")
        statusline = run(
            ["bash", "hooks/tokenzip-statusline.sh"],
            env=hook_env,
        )
        ensure("[TOKENZIP:WENYAN-ULTRA]" in statusline.stdout, "statusline badge output mismatch")
        ...
        ensure(settings_after == existing_settings, "uninstall.sh did not restore non-tokenzip settings")
        ensure(not (claude_dir / ".tokenzip-active").exists(), "uninstall.sh should remove flag file")

    with tempfile.TemporaryDirectory(prefix="tokenzip-verify-fresh-") as temp_root:
        ...
        activate = run(["node", "hooks/tokenzip-activate.js"], env=hook_env)
