#!/usr/bin/env python3
"""Benchmark tokenzip vs normal Claude output token counts."""
...
SKILL_PATH = REPO_DIR / "skills" / "tokenzip" / "SKILL.md"
...
def run_benchmarks(client, model, prompts, tokenzip_system, trials):
    results = []
    total = len(prompts)
    ...
    for i, prompt_entry in enumerate(prompts, 1):
        ...
        for mode, system in [("normal", NORMAL_SYSTEM), ("tokenzip", tokenzip_system)]:
...
def compute_stats(results):
    ...
    for entry in results:
        ...
        tokenzip_medians = statistics.median(
            [t["output_tokens"] for t in entry["tokenzip"]]
        )
        savings = 1 - (tokenzip_medians / normal_medians) if normal_medians > 0 else 0
        ...
        rows.append(
            {
                ...
                "tokenzip_median": int(tokenzip_medians),
    ...
    avg_tokenzip = round(statistics.mean([r["tokenzip_median"] for r in rows]))
...
def format_table(rows, summary):
    ...
    for r in rows:
        ...
        lines.append(
            f"| {label} | {r['normal_median']} | {r['tokenzip_median']} | {r['savings_pct']}% |"
        )
    lines.append(
        f"| **Average** | **{summary['avg_normal']}** | **{summary['avg_tokenzip']}** | **{summary['avg_savings']}%** |"
    )
    lines.append("")
    lines.append(
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark tokenzip vs normal Claude")
    ...
    tokenzip_system = load_tokenzip_system()
    ...
    results = run_benchmarks(client, args.model, prompts, tokenzip_system, args.trials)
